import { Button, Checkbox, Form, Input, message, Modal, Table, Radio, Select } from "antd"
...
const SetStandard = ({
  ...
  //const [autoGrading, setIsChecked] = useState(false);
  const [digitalPin, setDigitalPin] = useState(0);
  const [interruptPin, setInterruptPin] = useState(0);
  const [digitalOutputPin, setDigitalOutputPin] = useState(0);
...
//These two functions are used to handle changes in criteria name and rating
...
//Depending on the number user entered, show input field for both rating and criteria, so name could be entered.
...
//Use for output the name of criteria and rating in the table
const columns = [
  { 
...
//handle individual grid, which is empty
